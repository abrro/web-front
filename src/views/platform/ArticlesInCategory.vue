<template>
  <b-container>
    ...
    <h1 v-else> No articles in this category yet</h1>
...
export default {
  ...
  watch:{
    currentPage: function(page){
      this.$axios.get(`/api/category/${this.$route.params.label}/articles/page/${page}`).then((response) => {
  ...
  created() {
    ...
    this.$axios.get(`api/category/${this.$route.params.label}/articles/page/${this.currentPage}`).then((response) => {
      this.articles = response.data;
    });
